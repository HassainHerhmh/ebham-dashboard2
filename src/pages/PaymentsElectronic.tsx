...
const ElectronicPayments: React.FC = () => {
  const [methods, setMethods] = useState<ElectronicMethod[]>([]);
  ...
  const loadMethods = async () => {
    const res = await api.electronicPayments.getAll(); // API خاص بالدفع الإلكتروني
  ...
  const saveMethod = async () => {
    ...
    if (editingId) {
      await api.electronicPayments.update(editingId, payload);
    } else {
      await api.electronicPayments.add(payload);
    }
  ...
  const handleDragEnd = async (event: any) => {
    ...
    await api.electronicPayments.reorder(
      newList.map((m: ElectronicMethod, i: number) => ({
        id: m.id,
        sort_order: i + 1,
      }))
    );
  ...
  return (
    <div className="bg-white p-6 rounded shadow space-y-4">
      <div className="flex justify-between">
        <h2 className="text-lg font-bold">الدفع الإلكتروني</h2>
        <button
          onClick={() => {
            resetForm();
            setModalOpen(true);
        ...
        <div className="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50">
          <div className="bg-white p-6 rounded w-full max-w-md space-y-3">
            <h3 className="font-bold">
              {editingId ? "تعديل وسيلة" : "إضافة وسيلة دفع إلكترونية"}
            </h3>

            <input
              className="border p-2 w-full rounded"
              placeholder="اسم الجهة (بوابة الدفع)"
              value={company}
