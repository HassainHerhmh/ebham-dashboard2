...
const TransitAccountsSettings = () => {
  ...
  useEffect(() => {
    (async () => {
      const res = await api.get("/accounts/list"); // أو المسار الذي تجلب به الحسابات
  ...
  const save = async () => {
    const payload = {
      commission_income_account: commissionIncome,
      courier_commission_account: courierCommission,
      transfer_guarantee_account: transferGuarantee,
      currency_exchange_account: currencyExchange,
    };
  ...
  return (
    <div className="space-y-6" dir="rtl">
      <h2 className="text-lg font-bold text-green-700">الحسابات الوسيطة (Transit)</h2>
